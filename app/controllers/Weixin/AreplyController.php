...
{
    public function  postCautoreply(){
        ...
        if($arr["type"]=="text"){
            ...
            $msg_ids = Autoreply::where("mp_id",$arr["mp_id"])->where("msg_id",$arr["content"])->lists("msg_id");
            if(count($msg_ids)>0){
    ...
    public function  postUautoreply(){
        $json = file_get_contents('php://input');
        $arr = json_decode($json,true);
        for($i = 0;$i<count($arr["keyword"])-1;$i++){
            for($j = $i+1;$j<count($arr["keyword"]);$j++){
