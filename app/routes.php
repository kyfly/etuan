...
{
    $reg_serial = Registration_user::whereIn('reg_id',array(81,72,67,65,61,15,29,45,55,59))
                    ->orderBy('reg_serial')
                    ->lists('reg_serial');
    foreach ($reg_serial as $key => $value) {
        ...
        Reg_result::where('reg_serial',$value)->where('question_id',1)
        ...
        Reg_result::where('reg_serial',$value)->where('question_id',2)
...
{
    $reg_serial = Registration_user::whereIn('reg_id',array(79,74,64,35,36,42,60))
                    ->orderBy('reg_serial')
                    ->lists('reg_serial');
    foreach ($reg_serial as $key => $value) {
        $wx_uid = Registration_user::where('reg_serial',$value)->pluck('wx_uid');
        $wx_user_info =WxUser::where('wx_uid',$wx_uid)->select('stu_name','stu_id')->first();
        Reg_result::where('reg_serial',$value)->where('question_id',1)
                        ->update(array('answer'=>$wx_user_info->stu_name));
        Reg_result::where('reg_serial',$value)->where('question_id',2)
                ->update(array('answer'=>$wx_user_info->stu_id));
    }
});